...
// 1. Updated import to use ZxcvbnFactory
  ...
  return (
    <PasswordInputContext value={{ password: currentValue.toString() }}>
      <div className="space-y-3">
        <InputGroup className="rounded-sm font-robo">
          <InputGroupInput
            {...props}
            value={value}
...
export function PasswordInputStrengthChecker() {
  // 2. Track the instance directly instead of a boolean flag
  const [zxcvbnInstance, setZxcvbnInstance] = useState<ZxcvbnFactory | null>(null)
  ...
  const strengthResult = useMemo(() => {
    // 3. Fallback early if the library instance is not ready yet
    if (!zxcvbnInstance || deferredPassword.length === 0) {
      return { score: 0, feedback: { warning: undefined } } as const
    }

    // 4. Use the new .check() method on the instance
  ...
  useEffect(() => {
    Promise.all([
      ...
      .then(([common, english]) => {
        // 5. Initialize the factory instance with the configuration
  ...
  function getLabel() {
    ...
    if (!zxcvbnInstance) return "Loading strength checker"
